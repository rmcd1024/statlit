---
title: 'Statistically Literate Documents: I'
...
* A literate document is by definition reproducible --- it can contain a complete record, including documentation, of all steps from the processing of raw data to publication.

* Literate systems can provide output to html, pdf, docx, etc.
...
* Pandoc is a document conversion toolkit
* I will demonstrate markdown this week and LaTeX/R/knitr next week
    * The main ideas are the same when using LaTeX and markdown. The syntax is slightly different
    
## Markdown Example


These are the first few lines of the previous slide:

```
## Creating Literate Documents

* There are many ways to create literate documents
* Two common paths:
    * [Markdown](https://daringfireball.net/projects/markdown/) plus [Pandoc](http://pandoc.org/)
    * LaTeX and R (with the knitr package). 
* A markdown document is a simple, readable text file that can be processed to yield nicely formatted output
...
* The [knitr documentation](http://yihui.name/knitr/demo/engines/) provides sample scripts for Python, Awk, Ruby, Haskell, Bash, Perl, Graphviz, TikZ, SAS, Scala, and CoffeeScript
...
* Here is an explanation of the code. Note that it is vectorized; there are no explicit loops
...
* You can incorporate all calculations,  output, and explanations into a single document
...
* We did not discuss the YAML header...
